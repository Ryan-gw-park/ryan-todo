  ...
  return (
    <>
      ...
      <Table
        headers={['영역', '내용']}
        rows={[
          ['내 이름 (예: Ryan Park)', '나에게 배정된 할일. 지금 할일 / 다음 할일로 구분'],
      ...
      <H3>프로젝트 필터</H3>
      <P>오른쪽 상단의 [전체 | 팀 | 개인] 필터로 표시할 프로젝트를 전환할 수 있습니다. 이 필터는 매트릭스뿐 아니라 지금 할일, 프로젝트, 타임라인 뷰에서도 동일하게 적용됩니다.</P>
      ...
      <Table
        headers={['동작', '결과']}
        rows={[
          ['남은 할일 → 내 지금/다음', '나에게 자동 배정'],
          ['내 카드 → 남은 할일', '배정 해제 (미배정 팀 할일로)'],
          ['내 카드 → 다른 팀원 행', '해당 팀원에게 배정 + "다음 할일"로 배치'],
          ['같은 셀 안에서 드래그', '순서 변경'],
        ]}
      />
      <Tip>카드를 다른 팀원에게 드래그하면 자동으로 "다음 할일"에 배치됩니다. 상대방이 직접 "지금 할일"로 올릴지 결정할 수 있습니다.</Tip>
...
function SectionTasks() {
  return (
    <>
      ...
      <H3>할일 생성</H3>
      <P>각 프로젝트 셀의 "+ 추가" 버튼을 클릭하거나, 상단의 "+ 새 할일" 버튼으로 생성할 수 있습니다. 내 섹션(지금/다음)에서 생성하면 자동으로 나에게 배정됩니다.</P>
