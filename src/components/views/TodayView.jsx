...
export default function TodayView() {
  ...
  const todayCat = categories.find(c => c.name.includes('오늘') || c.name.toLowerCase().includes('today')) || categories[0] || null
  ...
  return (
    <div className="p-3 md:p-4">
      ...
      <div className="flex flex-col gap-2 max-w-[760px]">
        {projects.map(proj => {
          const todayTasks = tasks.filter(t => t.projectId === proj.id && t.categoryId === todayCat.id)
          const remTasks = tasks.filter(t => t.projectId === proj.id && t.categoryId !== todayCat.id)
          if (!todayTasks.length && !remTasks.length) return null
          ...
          return (
            ...
            >
              <div className="flex items-center gap-[9px] px-3.5 py-[10px] border-b border-[rgba(55,53,47,.09)] cursor-default rounded-t-md">
                ...
                <div className="flex flex-col gap-[2.5px] cursor-grab p-[5px] rounded opacity-0 group-hover:opacity-50 hover:opacity-100 hover:bg-[rgba(55,53,47,.08)] transition-opacity">
                  <div className="w-[10px] h-[1.5px] bg-[rgba(55,53,47,.65)] rounded-sm" />
                  <div className="w-[10px] h-[1.5px] bg-[rgba(55,53,47,.65)] rounded-sm" />
                  <div className="w-[10px] h-[1.5px] bg-[rgba(55,53,47,.65)] rounded-sm" />
                </div>
                <button onClick={() => openModal({ projectId: proj.id, categoryId: todayCat.id })} className="px-[9px] py-1 text-[11px] border border-[rgba(55,53,47,.16)] rounded bg-transparent cursor-pointer text-[rgba(55,53,47,.65)] hover:bg-[rgba(55,53,47,.04)]">+ 추가</button>
              </div>
              <div className="p-[3px_8px_8px]">
                {todayTasks.length ? todayTasks.map(t => (
                  <TaskRow key={t.id} task={t} onToggle={() => toggleDone(t.id)} onOpen={() => openDetail(t.id)} />
                )) : (
                  <button onClick={() => openModal({ projectId: proj.id, categoryId: todayCat.id })} className="flex items-center gap-[7px] py-2 px-2 rounded w-full text-left border-none bg-transparent cursor-pointer text-[rgba(55,53,47,.4)] text-[13px] hover:bg-[rgba(55,53,47,.04)]">
