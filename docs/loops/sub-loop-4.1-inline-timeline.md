# Sub-Loop 4.1: 글로벌 타임라인 인라인 뷰 생성 + 라우팅 교체

아래 명령을 순서대로 실행하라. 코드를 자의적으로 해석하거나 추가 수정하지 마라.

---

## Step 1: 새 파일 생성 — src/components/views/InlineTimelineView.jsx
...
**주의**: 파일 하단에 `weeksBetween`이 두 번 정의되어 있다. 파일 상단의 standalone 함수 하나만 남기고, 파일 하단(GanttBar 아래)의 중복을 제거하라.

---

## Step 2: App.jsx 라우팅 교체
...
### 수정 2-3: personal-timeline 라우팅 변경
...
### 수정 2-4: PersonalTimelineView preload 제거 (있다면)

App.jsx에 PersonalTimelineView preload가 있으면 InlineTimelineView로 교체:

old_str (정확히 매칭되는 경우에만):
```
...
- [ ] 사이드바 → 팀 > 타임라인 클릭 → 인라인 타임라인 렌더 (좌우 분리 없는 통합 레이아웃)
- [ ] 깊이 필터 '프로젝트' → 프로젝트 행만 표시
- [ ] 깊이 필터 '+마일스톤' → 프로젝트 아래 MS 행 표시 (depth별 들여쓰기)
- [ ] 깊이 필터 '+마일스톤+할일' → MS 아래 할일 행 표시
- [ ] ABI 코리아 같은 deep 프로젝트: 법인설립 > 지점설립 > 필요서류 확보 계층이 3단 들여쓰기로 표시
- [ ] 정기주총 같은 flat 프로젝트: depth 0 MS → 바로 할일
- [ ] ▸/▾ 클릭으로 개별 접기/펼치기 동작
- [ ] 주간 날짜 헤더 (3/9, 3/16, 3/23...)
- [ ] 오늘 빨간 세로선 표시
- [ ] 각 행에 담당자 아바타 표시
- [ ] 사이드바 → 개인 > 타임라인 → scope="personal" 적용 (내 담당 MS/할일만)
- [ ] 기존 프로젝트 뷰의 TimelineView(projectId) 사용은 영향 없음 (기존 TimelineEngine 그대로)
- [ ] npm run build 성공
